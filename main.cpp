...
public:
  ...
  void ParseCommandLine(int argc, char **argv) {
    ...
    while(1) {
      ...
      int option_index = 0;
      int c = getopt_long(argc, argv, "h:n:m:t:d:p:o:i:k:w:?", long_options, &option_index);
      if (c == -1) break;
      switch (c) {
        case 'h': {
          host = optarg;
...
extern "C" int GetIPListNonStd(void *thread, char *requestedHostname, addr_t *addr, int max, int ipv4, int ipv6);
...
public:
  struct FlagSpecificData {
      ...
      std::vector<addr_t> cacheNonStd; // [MF] non std port
      time_t cacheTime;
      unsigned int cacheHits;
      FlagSpecificData() : nIPv4(0), nIPv6(0), nonStdIPv4(0), nonStdIPv6(0), cacheTime(0), cacheHits(0) {}
  };
  ...
  void cacheHit(uint64_t requestedFlags, bool force = false) {
    ...
    if (force || thisflag.cacheHits * 400 > (thisflag.cache.size()*thisflag.cache.size()) || (thisflag.cacheHits*thisflag.cacheHits * 20 > thisflag.cache.size() && (now - thisflag.cacheTime > 5))) {
      ...
      thisflag.cacheNonStd.clear();
      thisflag.nIPv4 = thisflag.nonStdIPv4 = 0;
      thisflag.nIPv6 = thisflag.nonStdIPv6 = 0;
      thisflag.cache.reserve(ips.size());
      thisflag.cacheNonStd.reserve(ips.size());
      for (set<CService>::iterator it = ips.begin(); it != ips.end(); it++) {
        ...
        if ((*it).GetInAddr(&addr)) {
          ...
          if( a.port == GetDefaultPort() ) {
            ...
            thisflag.cacheNonStd.push_back(a);
            thisflag.nonStdIPv4++;
          }
...
extern "C" int GetIPListNonStd(void *data, char *requestedHostname, addr_t* addr, int max, int ipv4, int ipv6) {
  CDnsThread *thread = (CDnsThread*)data;
  ...
  thread->cacheHit(requestedFlags);
  auto& thisflag = thread->perflag[requestedFlags];
  unsigned int size = thisflag.cacheNonStd.size();
  unsigned int maxmax = (ipv4 ? thisflag.nonStdIPv4 : 0) + (ipv6 ? thisflag.nonStdIPv6 : 0);
  ...
  while (i+1<max) {
    ...
    do {
        bool ok = (ipv4 && thisflag.cacheNonStd[j].v == 4) ||
                  (ipv6 && thisflag.cacheNonStd[j].v == 6);
        if (ok) break;
        j++;
        if (j==size)
            j=i/2;
    } while(1);
    addr[i] = thisflag.cacheNonStd[j];
    thisflag.cacheNonStd[j] = thisflag.cacheNonStd[i/2];
    thisflag.cacheNonStd[i/2] = addr[i];
