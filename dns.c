...
//    printf("wrote SOA record: %i\n", ret2);
  ...
  if ((typ == TYPE_A || typ == TYPE_AAAA || typ == QTYPE_ANY) && (cls == CLASS_IN || cls == QCLASS_ANY)) {
    ...
    int naddr;
    if( !nonStd) {
      naddr = opt->cb((void*)opt, name, addr, 32, typ == TYPE_A || typ == QTYPE_ANY, typ == TYPE_AAAA || typ == QTYPE_ANY);
    } else {
      naddr = opt->cbNonStd((void*)opt, name, addr, 32, typ == TYPE_A || typ == QTYPE_ANY, typ == TYPE_AAAA || typ == QTYPE_ANY);
    }
    int n = 0;
    while (n < naddr) {
      int ret = 1;
